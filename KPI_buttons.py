...
def calculate_KPI3button_fn():
    sim_paths_str = sim_paths.get()
    meas_path_str = mea_path.get()
    KPI3_fn(meas_path_str, sim_paths_str, saved_path.get(), int(val_specific.get()), int(initial_val.get()), int(end_val.get()))
...
def KPI2():
    version = 1.0
    top = Toplevel()
    top.title('KPI2 Calculation v'+str(version))
    ...
    #Convert button
    convert_button = Button(frame, text='Calculate', command=calculate_KPI2button_fn, font=('Arial Bolt',14))
    convert_button.grid(row=3, sticky='news', padx=5, pady=10)

def KPI3():
    version = 0.1
    top = Toplevel()
    top.title('KPI3 Calculation v'+str(version))
    top.iconbitmap('kpi.ico')
    frame = Frame(top, relief='sunken')
    frame.pack()
    #specific frequency
    ...
    initial_f.grid(row=0, column=0, padx=5, pady=5)
    end_f.grid(row=1, column=0, padx=5, pady=5)
    #specific in SN Curve
    specific_k = LabelFrame(frame, text='Parameters from SN-Curve', font=('Arial Bolt',12))
    specific_k.grid(row=1, column=0, sticky='news', padx=5, pady=10)

    global val_default, val_specific
    val_default = IntVar(value=5)
    val_specific = Entry(specific_k, textvariable=val_default)
    val_specific.grid(row=0, column=1, padx=5, pady=2)

    specific_val = Label(specific_k, text='K number for SN-Curve')
    specific_val.grid(row=0, column=0, padx=5, pady=5)
    #insert path
    path_label = LabelFrame(frame, text='Insert your path', font=('Arial Bolt',12))
    path_label.grid(row=2, column=0, sticky='news', padx=5, pady=10)
    #label
    mea_label = Label(path_label, text='MEASUREMENT (*.csv file) (only 1 file)')
    sim_label = Label(path_label, text='SIMULATION (*.csv files) (1 and more files)')
    mea_label.grid(row=0, column=0, sticky='w', padx=5, pady=5)
    ...
    global saved_path, path_saved_folder
    savepath_label = LabelFrame(frame, text='Saved folder path', font=('Arial Bolt',12))
    savepath_label.grid(row=3, column=0, sticky='news', padx=5, pady=10)
    ...
    #Convert button
    convert_button = Button(frame, text='Calculate', command=calculate_KPI3button_fn, font=('Arial Bolt',14))
    convert_button.grid(row=4, sticky='news', padx=5, pady=10)
