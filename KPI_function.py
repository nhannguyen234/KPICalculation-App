        ...
        if end_f>initital_f:
            ...
            for filepath in paths_sim:
                ...
                if list(df_sim.columns) != new_column_order:
                    try:
                        df_sim[new_column_order].to_csv(filepath.split('.')[0] + '_updated.csv', index=False)
                        messagebox.showinfo(title='Info', message='File ' + filepath + ' is updated due to different order')
                    except:
                        messagebox.showerror(title='Error', message='Please close file ' + filepath.split('.')[0] + '_updated.csv')
                ###------
